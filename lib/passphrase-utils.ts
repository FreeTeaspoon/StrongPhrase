import { words, grammars as grammers_int } from "./words";
...
export const avgCostToCrack = (bits: number, costPerGuess32=0.56) => {
...
export function formatDollarToScale(amount: number, significantDigits=2, addScaleLabel=true) {
...
export const getWordStats = () => {
  ...
  for (const key in words) {
    countDict[key] = (words as Record<string, string[]>)[key].length;
  }
  return countDict;
}

export const getSampleWords = (listType: string, quantity=5) => {
  ...
  for (let i = 0; i < quantity; i++) {
    ...
    sampleWords.push((words as Record<string, string[]>)[listType][i*50+20]);
  }
  return sampleWords;
}
...
export const getAllGrammarLabels = () => {
  return Object.keys(grammars).reduce((acc: Record<string, string>, bits) => {
...
let getBitLength = function(bits: number, validate = true) {
  let bitLength = parseInt(String(bits));
  if (!validate) {
    return bitLength;
  }
  if(!(bitLength >= 1)) {
...
export const getPassphrase = (bits: number, grammars: Record<number, string> = DEFAULT_GRAMMARS, validate = true) => {        
  if(!cryptoAvailable()) {
      return "Error: Your browser cannot generate secure passphrases";
  }
  bits = getBitLength(bits, validate);
  const grammar = grammars[bits as keyof typeof grammars];
  ...
  if (cryptoObj) {
    cryptoObj.getRandomValues(randomNumbers);
  }
  ...
  components.forEach(function(component) {
      ...
      let wordBits: number = parts[1] ? parseInt(parts[1]) : -1;
      ...
      if (wordBits === 0) {
        ...
        const listLength = (words as Record<string, string[]>)[wordType].length;
        let randomNumber = randomNumbers[randomNumberIndex] % listLength;
        randomNumberIndex++;
        phrase.push((words as Record<string, string[]>)[wordType][randomNumber]);
        actualBits = actualBits + Math.log2(listLength);
      }
      // Regular case - validate bit length
      else if(wordBits >= 2 && wordBits <= 16) {
        if(!(words as Record<string, string[]>).hasOwnProperty(wordType) || (words as Record<string, string[]>)[wordType].length < (1 << wordBits)) {
          console.log("Error in grammar! Word type: "+ wordType+" // Word Bits: "+(words as Record<string, string[]>)[wordType].length + " // Expected length: " + (1 << wordBits));
          errorInGrammar = true;
          return;
        }
...
export const avgTimeToCrackFormatted = (bits: number, hashesPerSecond: number) => {
  return convertTimeToReadableFormat(timeToCrackAvg(bits, hashesPerSecond));
}

export const timeToCrackAvg = (bits: number, hashesPerSecond: number) => {
  const totalCombinations = Math.pow(2, bits);
  return totalCombinations / (2 * hashesPerSecond);
};

export const convertTimeToReadableFormat = (seconds: number) => {
  const units = [
  ...
  const roundToSigFig = (num: number, sigFig: number) => {
    if (num === 0) return 0;
