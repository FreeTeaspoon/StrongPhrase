...
const getSeededElement = <T>(array: T[], seed: string): T => {
...
export const generateAvatar = (name: NameInfo, specificProvider: string | null = null, enabledProviders: Record<string, boolean> | null = null) => {
  ...
  const availableProviders = enabledProviders
    ? Object.entries(avatarProviders)
        ...
        .reduce((acc, [key, value]) => ({ ...acc, [key]: value }), {} as Record<string, AvatarProvider>)
    : avatarProviders;
