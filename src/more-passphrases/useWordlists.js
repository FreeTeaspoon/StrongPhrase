...
import { wordlistSystems } from './wordlistConfig';

const wordlistCache = new Map();

export const useWordlists = () => {
  const [loadedWordlists, setLoadedWordlists] = useState(new Map());
      ...
      try {
        const promises = wordlistSystems.map(async (system) => {
          ...
          if (wordlistCache.has(system.id)) {
            return [system.id, wordlistCache.get(system.id)];
          }
  ...
  const getRandomWords = (systemId, numWords) => {
