...
const CopyableItem = ({ 
  ...
  sourceLink = null,      // Optional source link
  refreshAnimation = false, // Whether to show refresh animation
  additionalContent = null, // Additional JSX content to show between label and passphrase
  children                 // Optional additional content
}) => {
  if (!itemId) {
    itemId = Math.random().toString(36).substring(2, 8);
  }
    ...
    <>
      <div 
        className={`passphrase-block group ${
          isCopied ? 'copied' : ''
        } ${isHidden ? 'hide' : ''} ${refreshAnimation ? 'refresh-animation' : ''} ${className}`}
      >
        {showLabel && (
              ...
              <div className="label-container">
                ...
                {showInfoIcon && infoBits && (
                  <span className="ml-2 group">
                    <div className="tooltip mt-1" data-tip={typeof infoBits === 'number' ? `${infoBits} bits of entropy` : infoBits}>
                ...
                {sourceLink && (
                  <div className="tooltip" data-tip="Open source for this wordlist">
                    <a href={sourceLink} target="_blank" rel="noopener noreferrer" className="text-gray-500 text-sm mt-1 md:mt-0">
